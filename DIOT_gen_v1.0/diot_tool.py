...
# ---------- export batch TXT ----------
def export_batch_txt(path, rows):
    """
    Genera el .txt para carga batch SAT según tu ejemplo:

    No ponemos IVA en ninguna columna (nada de devoluciones).
    Sólo:
      0: TipoTercero
      1: TipoOperacion
      2: RFC
      11: Base16 (entero, redondeo .5 hacia arriba)
      53: "01"
    Total de campos por línea: 54 (índices 0..53)
    """
    COLS_TOTAL = 54      # en tu ejemplo, el "01" cae en el índice 53
    IDX_BASE16  = 11
    LAST_INDEX  = 53
    LAST_CONST  = "01"

    def round_half_up_to_int(x):
        return int(Decimal(str(to_num(x))).quantize(0, rounding=ROUND_HALF_UP))

    lines = []
    for r in rows:
        fields = [""] * COLS_TOTAL
        fields[0] = str(r.get("TipoTercero","04")).strip()
        fields[1] = str(r.get("TipoOperacion","85")).strip()
        fields[2] = str(r.get("RFC","")).strip()

        # Base16: entero con redondeo .5 hacia arriba
        b16 = round_half_up_to_int(r.get("Base16", 0))
        fields[IDX_BASE16] = str(b16) if b16 else ""

        # IVA: NO lo ponemos en ninguna columna (queda vacío)

        # cierra con constante "01"
        fields[LAST_INDEX] = LAST_CONST
...
class DIOTApp(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1180x700"); self.minsize(1000, 620)
    ...
    def _build_ui(self):
        ...
        ttk.Button(frmBtns, text="Exportar pendientes.csv", command=self.save_pendientes, state="disabled").pack(side="left", padx=(0,6))

        # Exportar batch .txt
        self.btn_export_batch = ttk.Button(frmBtns, text="Exportar Carga Batch (.txt)",
                                           command=self.save_batch_txt, state="disabled")
        self.btn_export_batch.pack(side="left")

        right = ttk.Frame(frmMid, width=380); right.pack(side="left", fill="y", padx=(10,0))
        ...
        self.treePend.column("Proveedor", width=240, anchor="w")
        ...
        ttk.Button(frmP, text="Eliminar pendiente", command=self.delete_selected_pending).pack(side="left", padx=(0,6))
    ...
    def run_process(self):
        ...
        # habilitaciones
        has_rows = bool(rows)
        self.btn_save_types.config(state=("normal" if has_rows else "disabled"))
        self.btn_save_catalog.config(state="normal")
        self.btn_add_rfc.config(state=("normal" if pendientes else "disabled"))
        self.btn_export_batch.config(state=("normal" if has_rows else "disabled"))
    ...
    def add_rfc_to_selected(self):
        ...
        # quitar de pendientes en UI e interno
        sel_id = sel[0]
        nombre_ui = self.treePend.item(sel_id, "values")[0]
        self.treePend.delete(sel_id)
        key = normalize_name(nombre_ui) or rfc
        self.pendientes.pop(key, None)

        messagebox.showinfo("Pendientes", "RFC agregado. Reprocesa para que se agrupe en DIOT.")

    def delete_selected_pending(self):
        """Eliminar un pendiente manualmente de la lista y del diccionario interno."""
        sel = self.treePend.selection()
        if not sel:
            messagebox.showinfo("Pendientes", "Selecciona un proveedor en la lista de pendientes.")
            return
        if not messagebox.askyesno("Eliminar pendiente", "¿Seguro que quieres eliminarlo de la lista? Esto no afecta el CSV original."):
            return
        sel_id = sel[0]
        nombre_ui = self.treePend.item(sel_id, "values")[0]
        self.treePend.delete(sel_id)
        key = normalize_name(nombre_ui)
        self.pendientes.pop(key, None)

    # ---- export batch .txt
    def save_batch_txt(self):
        out = filedialog.asksaveasfilename(defaultextension=".txt",
                                           initialfile=f"Batch_{self.periodo.get().strip()}.txt",
                                           filetypes=[("Texto","*.txt")])
        if not out: return
        try:
            export_batch_txt(out, self.result_rows)
            messagebox.showinfo("Carga Batch", f"Archivo generado:\n{out}")
        except Exception as e:
            messagebox.showerror("Carga Batch", f"No se pudo generar:\n{e}")
